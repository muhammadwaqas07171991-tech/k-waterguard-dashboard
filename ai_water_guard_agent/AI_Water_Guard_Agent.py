...
class Config:
    ...
    _repo_data_dir = AGENT_DIR.parent / "water_quality_data"
    DATA_DIR = Path(os.environ.get("WATER_QUALITY_DATA_DIR", str(_repo_data_dir if _repo_data_dir.exists() else (Path.home() / "water_quality_data"))))
    ...
    WORKSPACE_DASHBOARD_FILE = AGENT_DIR / "AI_Water_Guard_Agent_dashboard.html"
    HISTORICAL_DATA_DIR = AGENT_DIR / "south_korea_water_quality_history"
    ...
    STATIC_DASHBOARD_DATA_DIR = AGENT_DIR / "dashboard_static_data"
    ...
    HSPF_ALGAE_SCENARIO_DIR = AGENT_DIR / "HSPF_Algae_Bloom_SKorea" / "05_analysis" / "algae_scenarios"
    ALGAL_BLOOM_DATA_DIR = AGENT_DIR / "algal_bloom_data"
    ...
    MAP_SHAPEFILE = AGENT_DIR / "ctprvn.shp"
...
def load_cyanobacteria_records(logger=None):
    ...
    coord_lookup = {
        ...
        'Panmun': {'longitude': 128.029272, 'latitude': 35.151061},
    }
    if Config.CYANOBACTERIA_STATION_COORDS_FILE.exists():
        try:
            coords = pd.read_csv(Config.CYANOBACTERIA_STATION_COORDS_FILE)
            for _, row in coords.iterrows():
                station = str(row.get('station', row.get('Station', ''))).strip()
                if not station:
                    continue
                lon = pd.to_numeric(row.get('longitude', row.get('Longitude')), errors='coerce')
                lat = pd.to_numeric(row.get('latitude', row.get('Latitude')), errors='coerce')
                if pd.notna(lon) and pd.notna(lat):
                    coord_lookup[station] = {'longitude': float(lon), 'latitude': float(lat)}
        except Exception as exc:
            if logger:
    ...
    try:
        ...
        for column in station_df.columns:
            if not column.lower().endswith('_cyanobecteria'):
                continue
            station = column.rsplit('_', 1)[0].strip()
            values = pd.to_numeric(station_df[column], errors='coerce')
            for sample_date, value in zip(station_df['Date'], values):
                if pd.isna(sample_date) or pd.isna(value):
                    continue
                coords = coord_lookup.get(station, {})
                rows.append({
                    'sample_date': sample_date,
                    'station_code': f'CYANO_{station.upper()}',
                    'station_name': station,
                    'lake_name': 'Jinyang Lake / Namgang Dam',
                    'river_basin': 'Nakdong River',
                    'large_watershed': 'Nakdong River',
                    'middle_watershed': 'Namgang Dam',
                    'watershed_code': '2018',
                    'cyanobacteria_cells_ml': float(value),
        ...
        if logger:
            logger.warning(f"Cyanobacteria station data could not be loaded: {exc}")

    if Config.CYANOBACTERIA_LAKEWIDE_FILE.exists():
        try:
            lake_df = pd.read_csv(Config.CYANOBACTERIA_LAKEWIDE_FILE)
            lake_df['Date'] = pd.to_datetime(lake_df.get('Date'), errors='coerce')
            mean_lon = np.nanmean([coords.get('longitude', np.nan) for coords in coord_lookup.values()])
            mean_lat = np.nanmean([coords.get('latitude', np.nan) for coords in coord_lookup.values()])
            for _, row in lake_df.iterrows():
                sample_date = row.get('Date')
                if pd.isna(sample_date):
                    continue
                for label, column in [('Lake mean', 'Lake_mean_cells'), ('Lake max', 'Lake_max_cells')]:
                    value = pd.to_numeric(row.get(column), errors='coerce')
                    if pd.isna(value):
                        continue
                    rows.append({
                        'sample_date': sample_date,
                        'station_code': f'CYANO_{label.upper().replace(" ", "_")}',
                        'station_name': label,
                        'lake_name': 'Jinyang Lake / Namgang Dam',
                        'river_basin': 'Nakdong River',
                        'large_watershed': 'Nakdong River',
                        ...
                        'watershed_code': '2018',
                        'cyanobacteria_cells_ml': float(value),
                        'latitude': float(mean_lat) if pd.notna(mean_lat) else np.nan,
            ...
            if logger:
                logger.warning(f"Lakewide cyanobacteria data could not be loaded: {exc}")
...
class PlotGenerator:
    ...
    def __init__(self):
        ...
        plt.rcParams.update({
            ...
            'font.family': 'serif',
            'font.serif': ['Plus Jakarta Sans', 'Arial', 'DejaVu Sans'],
            'axes.titlesize': 16,
            'axes.labelsize': 13,
            'xtick.labelsize': 11,
            'ytick.labelsize': 11,
    ...
    def _cleanup_removed_plot_files(self):
        active_files = {
            ...
            'quality_heatmap.png',
            'distributions.png',
            'historical_trend_annual_means.png',
            'historical_sen_slope_summary.png',
    ...
    def _write_historical_trend_interactive(self, annual, variables):
        ...
        layout = {
            ...
            'paper_bgcolor': 'rgba(7, 21, 34, 0.95)', 'plot_bgcolor': 'rgba(4, 9, 14, 0.9)',
            'margin': {'l': 76, 'r': 34, 't': 92, 'b': 74},
            'xaxis': {'title': 'Year', 'gridcolor': 'rgba(255, 255, 255, 0.08)', 'zeroline': False},
            'yaxis': {'title': 'Annual mean', 'gridcolor': 'rgba(255, 255, 255, 0.08)', 'zeroline': False},
            'hovermode': 'closest',
            'updatemenus': [{
                'buttons': buttons,
                'direction': 'down',
    ...
    def _plot_algal_bloom_dashboard(self):
        try:
            ...
            if df.empty or 'cyanobacteria_cells_ml' not in df.columns:
                return
            df = df.dropna(subset=['cyanobacteria_cells_ml']).copy()
            if df.empty:
                return
            watershed_column = 'middle_watershed' if 'middle_watershed' in df.columns else 'river_basin'
            ...
            grouped = (
                ...
                .agg(
                    max_cells=('cyanobacteria_cells_ml', 'max'),
                    mean_cells=('cyanobacteria_cells_ml', 'mean'),
                    stations=('station_code', 'nunique'),
                    latest_date=('sample_date', 'max'),
                )
                .sort_values('max_cells', ascending=False)
                .head(15)
            )
            if not grouped.empty:
                fig, ax = self._new_figure(figsize=(13.4, 7.6))
                colors = self._coolwarm_values(grouped['max_cells'].values)
                ax.barh(grouped.index[::-1], grouped['max_cells'].values[::-1], color=colors[::-1], edgecolor='white')
                self._style_axis(ax, grid_axis='x')
                ...
                ax.set_title('Watershed Harmful Cyanobacteria Screening', fontweight='bold', fontsize=19, color=self.INK, pad=14)
                ax.set_xlabel(f'Observed maximum cyanobacteria ({rule["unit"]})')
                ax.legend(loc='lower right', frameon=True, facecolor='white', edgecolor='#d8e3f1')
                self._save_figure(fig, self._plots_dir() / 'algal_watershed_cyanobacteria.png', rect=[0, 0, 1, 0.98])

            df['year'] = df['sample_date'].dt.year
            annual_watershed = (
                df.groupby([watershed_column, 'year'])['cyanobacteria_cells_ml']
                .max()
                .reset_index()
            )
            ...
            station_max = (
                ...
                .agg(
                    ...
                    cyanobacteria_cells_ml=('cyanobacteria_cells_ml', 'max'),
                    latest_date=('sample_date', 'max'),
                )
            )
            if not station_max.empty and Config.WATERSHED_SHAPEFILE.exists():
                watershed_rings = self._read_polygon_shapefile(Config.WATERSHED_SHAPEFILE, Config.WATERSHED_SHAPEFILE_CRS)
                fig = plt.figure(figsize=(12.6, 8.1), facecolor=self.FIG_BG)
                ax = fig.add_axes([0.06, 0.1, 0.78, 0.78])
                cax = fig.add_axes([0.88, 0.18, 0.025, 0.62])
                ax.set_facecolor(self.AX_BG)
                self._draw_south_korea_map(ax, watershed_rings)
                color_values = np.log10(station_max['cyanobacteria_cells_ml'].clip(lower=1))
                sizes = np.clip(np.sqrt(station_max['cyanobacteria_cells_ml'].clip(lower=1)) * 5.5, 120, 680)
                scatter = ax.scatter(
                    station_max['longitude'], station_max['latitude'],
                    c=color_values, s=sizes,
                    cmap='coolwarm', edgecolors='white', linewidths=1.4, alpha=0.95, zorder=4,
                )
                label_offsets = {'Naedong': (-0.55, 0.24), 'Panmun': (0.12, -0.2)}
                for _, row in station_max.iterrows():
                    dx, dy = label_offsets.get(str(row['station_name']), (0.1, 0.1))
                    ax.annotate(
                        f"{row['station_name']}\n{row['cyanobacteria_cells_ml']:,.0f}",
                        ...
                        color=self.INK,
                        zorder=5,
                        arrowprops={'arrowstyle': '-', 'color': '#6f829a', 'linewidth': 0.9},
                        bbox={'boxstyle': 'round,pad=0.22', 'facecolor': 'white', 'edgecolor': '#d8e3f1', 'alpha': 0.94},
                    )
                ax.set_xlim(125.6, 131.1)
                ax.set_ylim(33.0, 39.4)
                ax.set_aspect('equal', adjustable='box')
                ax.set_title('Cyanobacteria Spatial Status On Korean Watershed Boundaries', fontweight='bold', fontsize=18, color=self.INK, pad=12)
...
<head>
  ...
  <style>
    html, body {{ margin: 0; min-height: 100%; background: #ffffff; font-family: 'Plus Jakarta Sans', sans-serif; color: #0f172a; }}
...
<body>
  ...
  <script>
    ...
    def _write_algal_interactive_plots(self, df, grouped, heatmap_data, rule):
        ...
        base_layout = {
            'font': {'family': 'Plus Jakarta Sans, sans-serif', 'color': '#e2e8f0', 'size': 14},
            'paper_bgcolor': 'rgba(7, 21, 34, 0.95)', 'plot_bgcolor': 'rgba(4, 9, 14, 0.9)',
            'margin': {'l': 120, 'r': 44, 't': 78, 'b': 84},
            'hoverlabel': {'font': {'family': 'Plus Jakarta Sans, sans-serif', 'size': 13}, 'bgcolor': '#ffffff', 'bordercolor': '#cbd5e1'},
        }
        ...
        if heatmap_data is not None and not heatmap_data.empty:
            ...
            layout.update({
                ...
                'xaxis': {'title': {'text': 'Year', 'standoff': 18}, 'tickangle': 0, 'gridcolor': 'rgba(255, 255, 255, 0.08)'},
                'yaxis': {'title': '', 'automargin': True, 'tickfont': {'size': 15}},
            })
    ...
    def _write_weather_interactive_plots(self, df, latest):
        ...
        layout.update({
            ...
            'yaxis': {'title': {'text': 'Daily precipitation (mm)', 'standoff': 16}, 'gridcolor': 'rgba(255, 255, 255, 0.08)', 'zerolinecolor': '#9eb4cc', 'linecolor': '#9eb4cc', 'mirror': True},
        })
...
class DashboardGenerator:
    ...
    def _assert_support_data_ready(self):
        ...
        message = (
            ...
            "Include dashboard_static_data/, algal_bloom_data/, watershed_shapes/, and HSPF algae scenario files in the deployment. "
...
<head>
  ...
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500;600&family=Outfit:wght@400;500;600;700;800;900&family=Space+Grotesk:wght@500;700&display=swap" rel="stylesheet">
  <style>
    :root {{
      color-scheme: dark;
      --bg-base: #04090e;
      --bg-surface: #071522;
      --bg-elevated: #0c2033;
      --bg-glass: rgba(8, 20, 32, 0.90);
      --bg-glass-strong: rgba(5, 13, 22, 0.96);
      --border-glass: rgba(255, 255, 255, 0.14);
      --border-glow: rgba(56, 189, 248, 0.45);
      --border-emerald: rgba(52, 211, 153, 0.45);
    ...
    *, *::before, *::after {{
    ...
    /* Subtle topological grid overlay matching portfolio */
    body::after {{
      content: '';
      position: fixed;
      inset: 0;
    ...
    /* Interactive ambient connected-nodes particle canvas */
    #ambient-canvas {{
      position: fixed;
      inset: 0;
      width: 100vw;
    ...
    header {{
      ...
      overflow: hidden;
      background:
        radial-gradient(1100px 420px at 15% 10%, rgba(56, 189, 248, 0.28) 0%, transparent 70%),
        radial-gradient(900px 380px at 85% 0%, rgba(52, 211, 153, 0.22) 0%, transparent 60%),
        radial-gradient(700px 320px at 50% 100%, rgba(0, 242, 254, 0.16) 0%, transparent 70%),
        linear-gradient(135deg, #04090e 0%, #071522 50%, #0b1f32 100%);
      color: #ffffff;
    ...
    .brand-logo {{
      ...
      object-fit: contain;
      border-radius: 12px;
      background: linear-gradient(135deg, rgba(56, 189, 248, 0.25), rgba(52, 211, 153, 0.3));
    ...
    /* Portfolio status-pill and kicker badges */
    .status-pill {{
      display: inline-flex;
      align-items: center;
      gap: 10px;
      ...
      box-shadow: 0 4px 18px rgba(0, 0, 0, 0.4), 0 0 14px rgba(16, 185, 129, 0.22);
    ...
    .badge, .kicker {{
      ...
      backdrop-filter: blur(16px);
      -webkit-backdrop-filter: blur(16px);
      box-shadow: 0 4px 18px rgba(0, 0, 0, 0.4), 0 0 14px rgba(56, 189, 248, 0.2);
      text-shadow: 0 1px 2px rgba(0, 0, 0, 0.8);
    }}
    ...
    header h1 {{
      ...
      line-height: 1.1;
      background: linear-gradient(135deg, #ffffff 40%, #bae6fd 75%, #a7f3d0 100%);
    ...
    .language-switch a.active {{
      background: linear-gradient(135deg, rgba(56, 189, 248, 0.35), rgba(52, 211, 153, 0.3));
      border: 1px solid rgba(56, 189, 248, 0.5);
      color: #ffffff;
    ...
    /* Main Container & Sticky Glass Navigation */
    main {{
      padding: 24px 0 60px;
    }}
    ...
    body .nav-tabs {{
      ...
      border: 1px solid var(--border-glass);
      border-radius: var(--radius-pill);
      box-shadow: var(--shadow-md), 0 0 24px rgba(0, 0, 0, 0.6);
      margin: 0 0 26px;
    }}
    ...
    body.page-contact .nav-tabs a[href="contact.html"] {{
      ...
      background: linear-gradient(135deg, rgba(56, 189, 248, 0.35), rgba(52, 211, 153, 0.3)) !important;
    ...
    .page {{ display: block; }}
    body.page-home .page:not(.page-home) {{ display: none !important; }}
    body.page-trends .page:not(.page-trends) {{ display: none !important; }}
    body.page-algal .page:not(.page-algal) {{ display: none !important; }}
    body.page-weather .page:not(.page-weather) {{ display: none !important; }}
    body.page-agro .page:not(.page-agro) {{ display: none !important; }}
    body.page-contact .page:not(.page-contact) {{ display: none !important; }}

    body.page-home .toolbar,
    body.page-home .search-status {{ display: none !important; }}

    body.page-contact #historicalDownloads,
    body.page-contact #historicalDataPage {{ display: block !important; }}
    ...
    body.page-trends #stationMeasurements {{ display: block !important; }}

    body:not(.page-contact) #installAppButton {{ display: none !important; }}
    body.page-contact #installAppButton {{ display: inline-flex !important; }}
    body:not(.page-contact) .toolbar a.button {{ display: none !important; }}
    body.page-contact .toolbar a.button {{ display: inline-flex !important; }}
    ...
    .search:focus {{
      ...
      box-shadow: 0 0 0 3px rgba(56, 189, 248, 0.25), 0 0 24px rgba(56, 189, 248, 0.25);
    }}
    ...
    .button.secondary:hover {{
      ...
      border-color: var(--cyan);
      color: #38bdf8;
      box-shadow: 0 8px 26px rgba(0, 0, 0, 0.5), 0 0 20px rgba(56, 189, 248, 0.3);
    }}

    /* Home Overview / Hero Panel */
    .home-overview {{
      display: grid;
      ...
      align-items: stretch;
      background:
        radial-gradient(circle at 10% 15%, rgba(56, 189, 248, 0.22) 0%, transparent 55%),
        radial-gradient(circle at 85% 85%, rgba(52, 211, 153, 0.15) 0%, transparent 50%),
        linear-gradient(135deg, #071522 0%, #0a1f33 50%, #0c2842 100%);
    ...
    .stat:hover {{
      ...
      box-shadow: var(--shadow-md), 0 0 25px rgba(56, 189, 248, 0.2);
    ...
    .section h2 {{
      ...
      align-items: center;
      gap: 12px;
      background: linear-gradient(135deg, #ffffff 50%, #93c5fd 85%, #6ee7b7 100%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
    ...
    .capability-card:hover {{
      border-color: var(--border-glow);
      box-shadow: var(--shadow-md), 0 0 25px rgba(56, 189, 248, 0.18);
    ...
    .param:hover {{
      transform: translateY(-3px);
      border-color: var(--cyan);
      box-shadow: 0 8px 24px -2px rgba(56, 189, 248, 0.25);
    }}

    .param.active {{
      border-color: var(--cyan);
      background: rgba(14, 34, 56, 0.95);
      box-shadow: 0 0 0 2px rgba(56, 189, 248, 0.4), 0 0 20px rgba(56, 189, 248, 0.2);
    }}
    ...
    .plot:hover, .spatial-map:hover {{
      border-color: var(--border-glow);
      box-shadow: var(--shadow-md), 0 0 25px rgba(56, 189, 248, 0.16);
    }}

    .plot h3, .spatial-map h3 {{
      font-family: var(--font-heading);
      font-size: 1.15rem;
      font-weight: 700;
      color: #ffffff;
    ...
    .app-install-panel {{
      ...
      align-items: center;
      background:
        radial-gradient(circle at 85% 20%, rgba(56, 189, 248, 0.28) 0%, transparent 60%),
        linear-gradient(135deg, #071522 0%, #0c2033 60%, #102e4c 100%);
      border-radius: var(--radius-lg);
      padding: 34px;
      color: #ffffff;
      margin: 22px 0 28px;
    ...
    @media (max-width: 1280px) {{
      ...
      .plots, .spatial-maps {{ grid-template-columns: 1fr; }}
    ...
    @media (max-width: 900px) {{
      ...
      .objective-grid, .agro-sector-grid {{
        grid-template-columns: 1fr !important;
      }}
...
<body class="page-home">
  ...
  <script>
    ...
    // --------------------------------------------------------------------
    // Environmental AI Interactive Connected-Nodes Canvas Engine
    // --------------------------------------------------------------------
    (function () {{
      ...
      const PIGMENTS = [
        {{ r: 56, g: 189, b: 248 }},  // Electric Cyan (Hydrology / Rivers)
        {{ r: 0, g: 242, b: 254 }},   // Aqua (AI Nexus)
        {{ r: 52, g: 211, b: 153 }},  // Emerald (Vegetation / Agriculture)
        {{ r: 129, g: 140, b: 248 }}, // Violet (CMIP6 / Atmosphere)
        {{ r: 251, g: 191, b: 36 }}   // Amber (Alert Warning / Solar)
      ];
      ...
      class WaterNode {{
        constructor(w, h) {{
          ...
          this.color = PIGMENTS[Math.floor(Math.random() * PIGMENTS.length)];
        ...
        draw() {{
          ...
          // Ripple ping on packet arrival
          if (this.pingAlpha > 0) {{
            ctx.beginPath();
            ctx.arc(this.x, this.y, this.pingRadius, 0, Math.PI * 2);
            ctx.strokeStyle = `rgba(${{r}}, ${{g}}, ${{b}}, ${{this.pingAlpha}})`;
            ctx.lineWidth = 1.4;
            ctx.stroke();
          }}
        }}
      ...
      let nodes = [];
      let packets = [];
      const MAX_NODES = Math.min(54, Math.floor(window.innerWidth / 26));
      ...
      function resize() {{
        ...
        if (nodes.length === 0) {{
          for (let i = 0; i < MAX_NODES; i++) {{
            nodes.push(new WaterNode(width, height));
          }}
        }}
      }}
      ...
      function loop() {{
        ...
        for (let i = 0; i < nodes.length; i++) {{
          for (let j = i + 1; j < nodes.length; j++) {{
            ...
            if (dist < maxDist) {{
              ...
              // Random packet spawning on conduits
              if (packets.length < 14 && Math.random() < 0.003) {{
                packets.push(new ProcessPacket(nodes[i], nodes[j]));
              }}
            }}
          }}
        }}

        // Draw Interactive Mouse Laser Conduits
        if (mouse.active) {{
          for (let i = 0; i < nodes.length; i++) {{
        ...
        for (let i = mouse.ripples.length - 1; i >= 0; i--) {{
          ...
          ctx.lineWidth = 1.6;
          ctx.stroke();
          if (rip.alpha < 0.02) mouse.ripples.splice(i, 1);
        }}
      ...
      window.addEventListener('click', (e) => {{
        mouse.ripples.push({{ x: e.clientX, y: e.clientY, radius: 5, alpha: 0.85 }});
      }});
    ...
    def _page_variants(self, html_text):
        return {
            ...
            'spatial.html': 'page-trends',
            'data.html': 'page-contact',
        }
  ...
  <section class="chat-panel" id="chatPanel" data-chat-url="{endpoint}" aria-label="K-Water Guard AI chatbot">
    ...
    def _algal_bloom_rows(self, limit=80, scope=None):
        ...
        rule = Config.ALGAL_BLOOM_RULES['cyanobacteria_cells_ml']
        working['status'] = working['cyanobacteria_cells_ml'].apply(lambda value: self._algal_status(value, rule))
        priority = {'Bloom outbreak': 0, 'Warning': 1, 'Caution': 2, 'Normal': 3, 'No data': 4}
        working['_priority'] = working['status'].map(priority).fillna(9)
        alert_rows = working[working['status'].isin(['Bloom outbreak', 'Warning', 'Caution'])].copy()
        display = alert_rows if not alert_rows.empty else working.sort_values('sample_date').groupby('station_code', as_index=False).tail(1)
        rows = []
        for _, row in display.sort_values(['_priority', 'cyanobacteria_cells_ml', 'sample_date'], ascending=[True, False, False]).head(limit).iterrows():
            status = str(row.get('status', 'Normal'))
            pill = 'critical' if status in {'Bloom outbreak', 'Warning'} else 'warning' if status == 'Caution' else 'ok'
            watershed = row.get('middle_watershed') or row.get('river_basin') or row.get('large_watershed') or ''
            station_label = f"{row.get('station_code', '')} - {row.get('station_name', '')}"
            date_text = pd.to_datetime(row.get('sample_date'), errors='coerce')
            date_text = date_text.strftime('%Y-%m-%d') if pd.notna(date_text) else ''
            rows.append(
                '<tr>'
                ...
                f'<td>{html.escape(str(station_label))}</td>'
                f'<td>{html.escape(str(watershed))}</td>'
                f'<td>{float(row.get("cyanobacteria_cells_ml", 0)):,.0f}</td>'
                f'<td>{html.escape(rule["unit"])}</td>'
                f'<td>{html.escape(str(row.get("basis") or rule["basis"]))}</td>'
                '</tr>'
            )
        return ''.join(rows)
    ...
    def _watershed_status_rows(self, scope=None):
        ...
        grouped = working.groupby(working[watershed_column].fillna('Unknown')).agg(
            ...
            max_cells=('cyanobacteria_cells_ml', 'max'),
            latest_date=('sample_date', 'max'),
        ).sort_values('max_cells', ascending=False).head(30)
        rule = Config.ALGAL_BLOOM_RULES['cyanobacteria_cells_ml']
        rows = []
        for watershed, row in grouped.iterrows():
            status = self._algal_status(float(row.max_cells), rule)
            pill = 'critical' if status in {'Bloom outbreak', 'Warning'} else 'warning' if status == 'Caution' else 'ok'
            rows.append(
                '<tr>'
                f'<td>{html.escape(str(watershed))}</td>'
                f'<td>{int(row.stations):,}</td>'
                f'<td>{float(row.max_cells):,.0f} {html.escape(rule["unit"])}</td>'
                f'<td><span class="alert-pill {pill}">{html.escape(status)}</span></td>'
                '</tr>'
            )
        return ''.join(rows)
  ...
  <aside class="insight-rail right" aria-label="Map and alert side insights">
    ...
    def _write_google_site_bundle(self, html_text, date_label):
        ...
        for source in [
            ...
            if source.exists():
                algal_data_dir.mkdir(parents=True, exist_ok=True)
                target = algal_data_dir / source.name
                shutil.copyfile(source, target)
                replacements[self._file_uri(source)] = self._site_asset_url(f"data/algal_bloom/{source.name}")
...
# ==================== MAIN ====================
def main():
    """Entry point for the AI Water Guard Agent."""
    ...
    print("Dashboard, charts, cyanobacteria alerts, and watershed maps saved.")
